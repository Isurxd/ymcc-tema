  ...
  const featuredSlides = banners.length > 0 
    ? banners.map((b, idx) => {
        ...
        if (b.linkUrl && b.linkUrl.includes("id=")) {
          ...
          linkedProduct = products.find(p => p.id === pid) || products[0];
        } else {
          linkedProduct = products[0];
        }
        return { id: idx + 1, image: b.image, product: linkedProduct || { stockAmount: 1 } };
      })
    ...
    return (
      <div className="min-h-screen bg-[#fff] pt-32 pb-24 px-6 md:px-12">
        <div className="max-w-6xl mx-auto">
          ...
          <button 
            onClick={() => { setSelectedProduct(null); setQuantity(1); setSelectedSize("L"); setCurrentImageIndex(0); }}
          ...
          <div className="bg-white border-2 border-black rounded-3xl flex flex-col lg:flex-row overflow-hidden shadow-brutal mb-20">
              ...
              <div className="relative flex-grow">
                 ...
                 {selectedProduct.stockAmount <= 0 && (
                   <div className="absolute inset-0 bg-black/60 flex items-center justify-center backdrop-blur-sm z-20">
                     <span className="font-anton text-4xl text-white rotate-[-15deg] border-4 border-red-500 text-red-500 px-6 py-2 rounded-xl">HABIS TERJUAL</span>
                   </div>
                 )}
              </div>
              {/* Thumbnail Gallery */}
            ...
            <div className="p-6 md:p-10 lg:w-1/3 flex flex-col justify-center bg-white">
              ...
              <div className="flex gap-3 mb-8">
                {["S", "M", "L", "XL"].map(size => (
                  <button 
                    ...
                    className={`w-12 h-12 rounded-full border-2 border-black font-poppins font-bold flex items-center justify-center transition-colors ${
                      selectedSize === size ? "bg-[#c1ff00]" : "bg-white hover:bg-gray-100"
                    }`}
                  >
                    {size}
                  </button>
              ...
              >
                {selectedProduct.stockAmount <= 0 ? "HABIS TERJUAL" : "ADD TO CART"}
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {products.filter(p => p.id !== selectedProduct.id).map(prod => (
                <div key={prod.id} className="bg-white border-2 border-black rounded-3xl overflow-hidden shadow-brutal cursor-pointer group" onClick={() => setSelectedProduct(prod)}>
    ...
    <div className="min-h-screen bg-white pt-32 pb-24">
      <div className="max-w-6xl mx-auto px-6 md:px-12">
        ...
        <div className="mb-20 grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:h-[600px]">
          {/* Left Large Image */}
          <div 
            className="lg:col-span-2 relative rounded-[2rem] overflow-hidden border-2 border-black cursor-pointer shadow-brutal h-[400px] lg:h-full group"
            onClick={() => setSelectedProduct(featuredSlides[currentSlide]?.product)}
          >
              ...
              <div className="relative h-[400px] w-full bg-gray-100 border-b-2 border-black flex items-center justify-center">
                ...
                {product.stockAmount <= 0 && (
                  <div className="absolute inset-0 bg-black/50 flex items-center justify-center backdrop-blur-sm z-20 pointer-events-none">
                    <span className="font-anton text-2xl text-red-500 border-4 border-red-500 px-4 py-1 rotate-[-15deg] rounded-lg bg-black/50">HABIS TERJUAL</span>
                  </div>
                )}
              </div>
